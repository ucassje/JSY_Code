...
def electric(x):
        return 0#U_solar(x)*dU_solar(x)/(cos(x)**2)+(U_solar(x)**2/cos(x))*dcos_1(x)+(1/v_Ae_0**2)*(Bol_k)/(Me*n(x))*(n(x)*temperature(x)*lntemperature(x)+temperature(x)*n(x)*lnn(x))-(1/v_Ae_0**2)*(Bol_k)/(Me)*dlnB(x)*temperature(x)+(1/v_Ae_0**2)*(Bol_k)/(2*Me)*dlnB(x)*temperature(x)+(1/v_Ae_0**2)*(2*Bol_k)/(Me*x)*temperature(x)
        ...
        for j in range(Nv):
            for i in range(Nv):
                    if r==0 or r==Nr-1 or i==0 or i==Nv-1:
                            ...
                            f_1[r*(Nv)*(Nv)+j*Nv+i]=f_temp6[r*(Nv)*(Nv)+j*Nv+i]-(U_solar(z[r])*dU_solar(z[r])/cos(z[r])+U_solar(z[r])**2*dcos_1(z[r])+(cos(z[r])/v_Ae_0**2)*(Bol_k)/(Me*Density_pre[r])*(Density_pre[r+1]*Temperature_pal[r+1]-Density_pre[r-1]*Temperature_pal[r-1])/(2*delz)-(cos(z[r])/v_Ae_0**2)*(Bol_k)/(Me)*dlnB(z[r])*Temperature_pal[r]+(cos(z[r])/v_Ae_0**2)*(Bol_k)/(2*Me)*dlnB(z[r])*Temperature_per[r]+(cos(z[r])/v_Ae_0**2)*(2*Bol_k)/(Me*z[r])*Temperature_pal[r]+(2/(Density_next[r]+Density_pre[r]))*(Density_next[r]*Bulk_next[r]-Density_pre[r]*Bulk_pre[r])/delt)*(delt/(2*delv))*(f_temp6[r*(Nv)*(Nv)+j*Nv+i+1]-f_temp6[r*(Nv)*(Nv)+j*Nv+i-1]) #f_temp6[r*(Nv)*(Nv)+j*Nv+i]-(2/(Density_next[r]+Density_pre[r]))*(Density_next[r]*Bulk_next[r]-Density_pre[r]*Bulk_pre[r])*(1/(2*delv))*(f_temp6[r*(Nv)*(Nv)+j*Nv+i+1]-f_temp6[r*(Nv)*(Nv)+j*Nv+i-1])
